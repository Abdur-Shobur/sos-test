...
function CartInput({ state, dispatch, data }) {
	// const currentID = state.selectedData.filter((x) => x.id === data.id)[0].id;
		...
		<div className="row">
			<div className="col-md-4">
				...
				<select
					...
					// value={state?.uniqueColor[0]?.color_id}
				>
			...
			<div className="col-md-4">
				...
				<select
					...
					aria-label="Default select example"
					// value={state?.findSize[0]?.id}
				>
					{/* {state.findSize.map((c) => (
						<option key={c.size_id} value={c.id}>
							{c.size_name}
						</option>
					))} */}
					{data?.sizes?.map((c, i) => (
					...
					{/* <option value="sdfsf">sdfssdf</option>
					<option value="sdfsf">sdfsf2</option>
					<option value="sdfsf">sdfsfr</option>
					<option value="sdfsf">sdfsf3</option> */}
				</select>
			</div>
