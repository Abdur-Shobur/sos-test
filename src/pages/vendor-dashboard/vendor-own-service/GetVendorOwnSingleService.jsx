...
const GetVendorOwnSingleService = () => {
		...
		<div>
			...
			<div className="content">
				<CommonBreadCrumbs
					...
					link={{ name: 'All Order', path: '/vendors-dashboard/service-order' }}
				/>
				{isLoading ? (
					<EditLoader />
				) : (
							...
							{orderSingleData?.status === 'revision' ||
								(orderSingleData?.status === 'progress' && (
									<button className="btn btn-primary">
										<Link
											style={{ color: 'white' }}
											to={`/vendor-dashboard/my-service/delivery/${orderSingleData?.id}`}
										>
											Delivery
										</Link>
									</button>
...
export default GetVendorOwnSingleService;
