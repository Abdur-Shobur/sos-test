...
import { GetAllVendorById } from '../../../api/admin/vendorApi';

const VendorProfileView = () => {
	const { id } = useParams();

	const { vendorData, isLoading } = GetAllVendorById(id);
	...
	return isLoading ? (
		...
		<div className="content">
			...
			<div className="breadcrumb-wrapper breadcrumb-contacts">
				...
				<div>
					<Link
						to={`/admin/edit-vendors/${vendorData.id}`}
					...
					<div className="col-lg-4 col-xl-3">
						<div className="profile-content-left profile-left-spacing">
								...
								<div className="text-center widget-profile px-0 border-0">
									...
									<div className="card-body">
										...
										<span className="badge badge-primary my-2">
											Balance: {vendorData?.balance || '00'} TK
										</span>
							...
							<div className="contact-info pt-4">
								...
								<p>{vendorData?.number2 || 'Not set'}</p>
								<p className="text-dark font-weight-medium pt-24px mb-2">
									Create Date
								</p>
								<p>{time(vendorData.created_at)}</p>
								<p className="text-dark font-weight-medium pt-24px mb-2">
									Update Date
								</p>
								<p>{time(vendorData.updated_at)}</p>
							</div>
						</div>
					</div>
