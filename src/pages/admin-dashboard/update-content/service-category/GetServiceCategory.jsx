...
function GetServiceCategory() {
  ...
  const { serviceCategoryData, refetch, isLoading } =
    GetAdminServiceCategoryData();
  ...
  const membernDataDelete = (id) => {
    const del = () =>
      http.delete(`admin/servicecategory/${id}`).then((res) => {
  ...
  return (
    <div className="ec-content-wrapper">
      <Helmet>
        <title>Admin Service Cateogory-SOS</title>
      </Helmet>
      <div className="content">
        <div className="breadcrumb-wrapper breadcrumb-contacts">
          <div>
            <h1>Admin Service Cateogory</h1>
          ...
          <div>
            <Link
              ...
              to="/admin/create-service-category"
            >
              Create Service Cateogory
            </Link>
          </div>
        </div>
                  ...
                  >
                    ...
                    {isLoading ? (
                      ...
                      <tbody style={{ verticalAlign: "middle" }}>
                        {serviceCategoryData?.message?.length > 0 ? (
                          serviceCategoryData?.message?.map((data, i) => (
                            <tr key={data?.id}>
                              <td>{i + 1}</td>
                              <td>{data?.name}</td>
                              <td>{data?.status}</td>
