...
const OrganizationTwoModal = ({ clickId, organizationTwoRefetch }) => {
	...
	useEffect(() => {
		const getDataEditData = async () => {
			setLoadingData(true);
			const res = await http.get(`/admin/organizationTwo/${clickId}`);
	...
	const handleSubmitData = async (e) => {
		...
		try {
			const data = await http.put(`/admin/organizationTwo/${clickId}`, state);

			if (data.data.status === 400) {
				tost(data?.data?.errors.name[0]);
			} else if (data.data.status === 200) {
				organizationTwoRefetch();
...
export default OrganizationTwoModal;
