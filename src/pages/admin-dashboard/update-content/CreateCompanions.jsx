...
const CreateCompanions = () => {
	...
	const handleSubmitCompanions = async (e) => {
		...
		try {
			const data = await http.post(`/admin/companion`, getData);

			if (data.data.status === 400) {
				tost(data?.data?.errors.name[0]);
			} else if (data.data.status === 200) {
				navigate('/admin/update-companions');
	...
	return (
		<div>
			<Helmet>
				<title>Add New Companinos-SOS</title>
			</Helmet>
			<div className="content">
				<CommonBreadCrumbs
					heading="Add New Companinos"
					link={{ name: 'All Companinos', path: '/admin/update-companions' }}
				/>
				<form
					data-aos="fade"
				...
				>
					...
					<div className="modal-footer px-4">
						...
						>
							<span style={{ marginRight: '2px' }}>Create Companions</span>{' '}
...
export default CreateCompanions;
