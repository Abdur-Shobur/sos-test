...
function AdminAddNewUser() {
  ...
  const addVendorHandler = async (e) => {
    ...
    setError("");
    try {
      const res = await http.post(`/user/store`, state, multipartConfig);
  ...
  return (
    ...
    >
      ...
      <div className="modal-footer px-4">
        <Link to={"/admin/all-users"} className="btn btn-secondary btn-pill">
          Cancel
        </Link>
        <button
          type="submit"
        ...
        >
          <span style={{ marginRight: "2px" }}>Save User</span>{" "}
