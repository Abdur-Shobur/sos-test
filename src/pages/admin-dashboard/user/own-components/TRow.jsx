...
function TRow({ i, data, page, refetch }) {
  ...
  const delete_handler = (e) => {
    const del = () =>
      http.delete(`/delete-user/${e}`).then((res) => {
        tost(res.data.message);
        refetch();
      });
  ...
  return (
    <tr>
      ...
      <td>
        <TableImage id={data?.id} path={USER_VIEW} src={data?.image} />
      </td>
      <td>
        <TableName id={data?.id} path={USER_VIEW} text={data?.name} />
      </td>
      <td>
